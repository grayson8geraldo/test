...
function detectDeliveryZone(address) {
    ...
    var moscowRe = /москва|подольск|балаших|химки|реутов|мытищ|любер|королёв|королев|красногорск|одинцов|пушкино|щёлков|щелков|долгопруд|зеленоград|солнечногорск|павловский посад|серпухов|видное|лобн|раменск|истра|ногинск|электросталь|орехово-зуев|сергиев посад|дзержинский|чехов(?![а-яё])|наро-фоминск|можайск|ступин|клин(?!ц)|дмитров(?!град)|домодедов|апрелевк|бронниц|воскресенск|дедовск|зарайск|кашир|коломн|лосино-петровск|лыткарин|озёры|озеры|пересвет|протвин|пущин|рузск|серебряные пруды|талдом|фрязин|шатур|электрогорск|юбилейн|яхром|подмоск|московская обл/;
    if (moscowRe.test(lower)) {
        return { zone: 'msk', name: 'Москва и МО', cost: 300 };
    }

    // Санкт-Петербург и Ленинградская область
    var spbRe = /санкт-петербург|петербург|питер|спб|ленинградская обл|гатчин|колпин|петергоф|кронштадт|пушкин(?![ао])|сестрорецк|павловск(?![а-яё])|красное село|ломоносов|всеволожск|выборг|тосн|кириш|тихвин|волхов|луг(?:а|$|,)|кингисепп|сосновый бор|приозерск|бокситогорск/;
    if (spbRe.test(lower)) {
